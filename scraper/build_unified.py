...
def normalize_addr(addr):
    """Normalize a street address to a consistent key for matching."""
    addr = str(addr or '').upper().strip().split(',')[0]
...
# Load clerk records
clerk_list = []
if Path(CLERK_PATH).exists():
    with open(CLERK_PATH) as f:
        raw = json.load(f)
        clerk_list = raw.get('records', []) if isinstance(raw, dict) else raw

print(f"  Tax: {len(tax_list):,}  Fire: {len(fire_list):,}  Clerk: {len(clerk_list):,}")

# Build address index for clerk records (address → list of clerk docs)
# Exclude releases — they signal a resolved distress, not an active one
_SKIP_CATS = {'release'}
clerk_by_addr = {}
for rec in clerk_list:
    if rec.get('cat') in _SKIP_CATS:
        continue
    addr = rec.get('prop_address', '') or ''
    if addr:
        key = normalize_addr(addr)
        clerk_by_addr.setdefault(key, []).append(rec)
...
for rn in all_rnums:
    ...
    # ── Clerk match (address-based) ──────────────────────────────────────
    prop = enrich(rn, prop)   # fill situs_address first so we can match it
    situs_key = normalize_addr(prop.get('situs_address', ''))
    matched_clerks = clerk_by_addr.pop(situs_key, []) if situs_key else []
    if matched_clerks:
        sources.append('clerk')
        prop['distress_factors'].append('clerk')
        prop['clerk_records'] = [
            {k: cr.get(k) for k in ('doc_type','cat','cat_label','doc_num','filed',
                                    'amount','clerk_url','owner_name','lienholder','flags','score')}
            for cr in matched_clerks
        ]
        prop['clerk_types']  = list({cr.get('cat_label') or cr.get('doc_type','') for cr in matched_clerks})
        prop['clerk_latest'] = max((cr.get('filed','') for cr in matched_clerks), default='')
        prop['clerk_score']  = max((cr.get('score',0) or 0 for cr in matched_clerks), default=0)
        base_score = max(base_score, prop['clerk_score'])
        for cr in matched_clerks:
            for fl in (cr.get('flags') or []):
                if fl not in prop['flags']: prop['flags'].append(fl)
        # Fill owner/mail from clerk if still missing
        first_c = matched_clerks[0]
        if not prop.get('owner_name'): prop['owner_name'] = first_c.get('owner_name') or first_c.get('owner','')
        if not prop.get('mail_address'): prop['mail_address'] = first_c.get('mail_address','')
        if not prop.get('mail_city'):    prop['mail_city']    = first_c.get('mail_city','')
        if not prop.get('mail_state'):   prop['mail_state']   = first_c.get('mail_state','TX')
        if not prop.get('mail_zip'):     prop['mail_zip']     = first_c.get('mail_zip','')
...
# ── Standalone clerk records (no matching tax/fire R-number) ─────────────────
print("Adding standalone clerk-only records...")
standalone_count = 0
for addr_key, clerks in clerk_by_addr.items():
    if not clerks:
        continue
    first = clerks[0]
    prop = {
        'r_number': '',
        'situs_address': first.get('prop_address',''),
        'situs_city': first.get('prop_city','') or 'Lubbock',
        'situs_state': 'TX',
        'situs_zip': first.get('prop_zip',''),
        'owner_name': first.get('owner_name') or first.get('owner',''),
        'mail_address': first.get('mail_address',''),
        'mail_city': first.get('mail_city',''),
        'mail_state': first.get('mail_state','TX'),
        'mail_zip': first.get('mail_zip',''),
        'assessed_value': None,
        'legal_description': '',
        'address_source': 'clerk',
        'sources': ['clerk'],
        'flags': [],
        'distress_factors': ['clerk'],
    }
    prop['clerk_records'] = [
        {k: cr.get(k) for k in ('doc_type','cat','cat_label','doc_num','filed',
                                'amount','clerk_url','owner_name','lienholder','flags','score')}
    ...
    prop['clerk_types']  = list({cr.get('cat_label') or cr.get('doc_type','') for cr in clerks})
    prop['clerk_latest'] = max((cr.get('filed','') for cr in clerks), default='')
    prop['clerk_score']  = max((cr.get('score',0) or 0 for cr in clerks), default=0)
    for cr in clerks:
        for fl in (cr.get('flags') or []):
            if fl not in prop['flags']: prop['flags'].append(fl)

    base_score = prop['clerk_score']
    prop['score'] = apply_combo_bonus(base_score, {'clerk'})
    prop['combo_bonus'] = prop['score'] - base_score
    prop['source_count'] = 1
    prop['tax_score'] = 0
    prop['fire_score'] = 0
    prop['distress_label'] = 'Clerk Record'

    complete, missing = completeness(prop)
    prop['is_complete'] = complete
    prop['missing_fields'] = missing

    unified.append(prop)
    standalone_count += 1

print(f"  Added {standalone_count:,} standalone clerk-only leads")
unified.sort(key=lambda x: x['score'], reverse=True)
...
has_clerk  = sum(1 for r in unified if 'clerk' in r.get('distress_factors',[]))
print(f"\n{'='*50}")
print(f"Unified: {len(unified):,} | Triple: {triple} | Multi: {multi}")
print(f"With clerk: {has_clerk:,} | Score=100: {at100} | Hot≥70: {hot} | Incomplete: {incomplete}")
print(f"Top 5:")
for r in unified[:5]:
